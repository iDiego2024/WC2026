...
export function AdminStatusView() {
  ...
  const handleTrigger = async (fnName: string) => {
    ...
    try {
      ...
      alert(`Sincronización '${fnName}' ejecutada con éxito.`);
      fetchLogs();
    } catch (e: any) {
      console.error(e);
      alert(`Error ejecutando '${fnName}': ${e.message}`);
    } finally {
    ...
    <div className="space-y-6 animate-in fade-in duration-500 pb-10">
      <header className="space-y-0.5 border-b border-border/50 pb-4 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 font-medium tracking-wide uppercase">Real-time scheduling and API health monitoring</p>
        </div>
        <div className="flex items-center gap-4 bg-secondary/50 p-2 rounded-lg border border-border">
          <div className="flex flex-col">
            <span className="text-[9px] uppercase font-bold text-slate-500">API Health Status</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="bg-card border-border">
          <CardHeader className="p-4 pb-2">
            <CardTitle className="text-[10px] font-black uppercase text-slate-500 tracking-wider">Quota Consumed (Last Logs)</CardTitle>
          </CardHeader>
          <CardContent className="p-4 pt-0">
            <div className="text-2xl font-black text-white font-mono">{apiCallsLast24h} <span className="text-xs font-normal text-slate-400">Requests</span></div>
        ...
        <Card className="bg-card border-border">
          <CardHeader className="p-4 pb-2">
            <CardTitle className="text-[10px] font-black uppercase text-slate-500 tracking-wider">Average Sync Latency</CardTitle>
          </CardHeader>
          <CardContent className="p-4 pt-0">
            ...
            <div className="text-[10px] text-slate-500 mt-2">Deno Edge runtime speed</div>
          </CardContent>
        </Card>

        <Card className="bg-card border-border">
          <CardHeader className="p-4 pb-2">
            <CardTitle className="text-[10px] font-black uppercase text-slate-500 tracking-wider">Sync Success Rate</CardTitle>
          </CardHeader>
          <CardContent className="p-4 pt-0">
            <div className="text-2xl font-black text-white font-mono">
              {logs.length > 0 ? `${Math.round(((logs.length - totalErrors) / logs.length) * 100)}%` : '100%'}
            </div>
            <div className="text-[10px] text-slate-500 mt-2">{totalErrors} errors detected in log window</div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-card border-border">
          ...
          <CardContent className="p-4 pt-0">
            ...
            <div className="text-[10px] text-slate-500 mt-2">Subscribed to public:sync_logs</div>
          </CardContent>
        </Card>
      </div>
        ...
        <div className="lg:col-span-4 space-y-4">
          <Card className="bg-card border-border">
            <CardHeader className="p-4 border-b border-border/50 bg-secondary/30">
              <CardTitle className="text-xs font-black uppercase text-slate-200">Force Sync Triggers</CardTitle>
            </CardHeader>
              ...
              {[
                ...
                <div key={fn.name} className="p-3 bg-secondary/50 border border-white/5 rounded-lg space-y-3">
                  ...
                  >
                    ...
                    {triggering[fn.name] ? 'Triggering...' : `Run ${fn.name}`}
                  </Button>
                </div>
              ))}
            </CardContent>
            ...
            <CardContent className="p-0 overflow-x-auto">
              {loading ? (
                <div className="p-6 text-center text-slate-500 flex items-center justify-center gap-2">
                  <RefreshCcw className="w-4 h-4 animate-spin text-primary" /> Loading executions logs...
                </div>
              ) : logs.length === 0 ? (
                <div className="p-6 text-center text-slate-500 italic">No sync logs found in database.</div>
              ) : (
