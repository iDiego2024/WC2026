  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-[calc(100vh-8rem)] gap-4 text-slate-400 animate-in fade-in duration-500">
        ...
        <p className="text-xs font-bold uppercase tracking-widest animate-pulse">{t('Cargando Partidos...', 'Loading Matches...')}</p>
      </div>
    );
  }
  ...
  if (!matches || matches.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-[calc(100vh-8rem)] gap-3 text-slate-400 animate-in fade-in duration-500">
        ...
        <h3 className="text-sm font-bold text-white tracking-wide">{t('Sin Partidos', 'No Matches')}</h3>
        <p className="text-xs max-w-sm text-center opacity-80">{t('No hay partidos programados.', 'No matches scheduled.')}</p>
      </div>
    );
  }
                ...
                return (
                  <div key={match.id} onClick={() => navigate(`/match/${match.id}`)} className="flex flex-col sm:flex-row sm:items-center justify-between p-2 bg-card rounded-md border border-border hover:border-primary/50 transition-colors cursor-pointer">
                    ...
                    <div className="flex flex-col sm:w-1/4">
                      <span className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">{format(new Date(match.date), 'MMM do')} • {match.group_name ? `${t('Grp', 'Grp')} ${match.group_name}` : match.stage}</span>
